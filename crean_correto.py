...
class CreareDataCollector:
    ...
    def _get(self, base, path, params=None):
        ...
        url = f"{base}{path}"
        resp = self.s.get(url, headers={"Authorization": f"Bearer {self.token}"}, params=params, timeout=60)
        resp.raise_for_status()
    ...
    def collect_all(self):
        ...
        infra = {"customerChildIds":self.CUSTOMER_CHILD_ID,"fromTimestamp":(now-timedelta(days=30)).isoformat()+"Z","toTimestamp":now.isoformat()+"Z"}
        self._save("infractions", self._get(self.BASE_BASIC, "/infractions", infra))
        jour = {"customerChildIds":self.CUSTOMER_CHILD_ID,"fromTimestamp":(now-timedelta(days=7)).isoformat()+"Z","toTimestamp":now.isoformat()+"Z"}
        self._save("journeys", self._get(self.BASE_BASIC, "/journeys", jour))
        self._save("tracking_latest", self._get(self.BASE_BASIC, "/tracking/latest", {"customerId": self.CUSTOMER_CHILD_ID}))

if ($MyInvocation.PSCommandPath) {
    [CreareDataCollector]::new().collect_all()
    Write-Host "✅ Coleta Creare finalizada!" -ForegroundColor Green
}
